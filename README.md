...
- **Remediation & Architecture Fix** ([`scripts/verify/`](scripts/verify/)):
  ...
  2. A shared `waitForPipelineQuiescence` coordinator (backfill `COMPLETED` **and** `incremental_lag_records === 0`) now guards Gates 2, 3 and 4, so counts are asserted only against a settled pipeline. Gate 2 asserts exact parity (`esCount === sourceCount - esRejectedDlqRecords`) with no tolerance band; Gate 4 polls for exactly 497 written / 3 quarantined.
  3. Gate 3 applies a bounded mutation burst *during* the receiver blackout (default 5s, `OUTAGE_DURATION_SEC`), requires the breaker to be observed `OPEN`/throttling, and verifies every mutated row lands in Elasticsearch by `_mget` version comparison after restoration. Downtime and recovery time in the report line are measured, not constants.
